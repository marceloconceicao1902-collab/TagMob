...
import { SINAPRO_DELIVERABLES, calculateSinaproBudget } from "../lib/sinapro-pricing";
...
async function main() {
  ...
  console.log("Semeando itens entregáveis Sinapro-SP...");
  for (const item of SINAPRO_DELIVERABLES) {
  ...
  console.log("✓ 45 Itens entregáveis da Tabela Sinapro-SP semeados");

  // ─── 10. ORÇAMENTOS CONTRATADOS (OrcamentoContratado) ────────────────────
  console.log("Semeando orçamentos contratados para empreendimentos...");
  const empReserva = await prisma.empreendimento.findFirst({ where: { id: "emp-001" } });
  if (empReserva) {
    const calc = calculateSinaproBudget({
      selectedDeliverableIds: ["ent-sin-201", "ent-sin-301", "ent-sin-307"],
      extraHoursByArea: { "hr-design": 10, "hr-programacao": 5 },
      descontoInteriorPct: 0.20, // 20% interior
      aplicaRefacao: false,
    });

    await prisma.orcamentoContratado.upsert({
      where: { id: "orc-emp-001" },
      update: {
        ...
        etapa1Paga: true,
        dataContrato: new Date("2024-05-01"),
      },
      create: {
        id: "orc-emp-001",
        empreendimentoId: empReserva.id,
        valorEtapa1Fixo: calc.valorEtapa1Fixo,
        valorVariavel: calc.valorPecasOpcionais + calc.valorHorasAdicionais,
        valorTotal: calc.valorTotal,
    ...
    console.log("✓ Orçamento contratado semeado para emp-001");
  }
